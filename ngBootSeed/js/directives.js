(function () {
	...
	function  jmNotTheDroids(){
		return{
			restrict: 'EA',
			template: `<h4 ng-hide="list.items.length"><i class="alert-danger"> {{ 'NOITEMS' | translate}} </i></h4>
			<h4 ng-show="archived.length === 0 && list.items.length > 0 && !ic.archived"><i class="alert-danger"> {{ 'ALLARCHIVED' | translate}} </i></h4>
			<h4 ng-show="archived.length === 0 && list.items.length > 0 && ic.archived"><i class="alert-danger"> {{ 'NOARCHIVED' | translate}} </i></h4>`
		}
	}
